...
{
    int dx = q.x - p.x, ax = abs(dx),
    dy = q.y - p.y, ay = abs(dy);
...
{
    return sqrt(pow((double)(q.x-p.x),2) + pow((double)(q.y-p.y),2));  //pcompany2009
}
...
{
    return atan2((double)(b.y - a.y), (double)(b.x - a.x));  //pcompany2009
}
...
CIPoint CIFunction::closest(const CIPoint& p1, const CIPoint& p2, const CIPoint& p3)
{
    ...
    double m = double(p2.y - p1.y) / d;

    if (m == 0)
         return CIPoint(p3.x, p1.y);

    double b1 = p2.y - m * p2.x,
          b2 = p3.y + 1/m * p3.x,
          x = (b2 - b1) / (m + 1/m), 
          y = m * x + b1;

    return CIPoint(int(x),int(y));
}

double CIFunction::cross(const CIVector& a, const CIVector& b) {
    int dx1 = a.endp.x - a.startp.x, dx2 = b.endp.x - b.startp.x,
