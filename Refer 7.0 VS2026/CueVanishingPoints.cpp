...
{
	POINT2D vp (0, 0);
	long n= (long)Cluster[numCluster].size();
	for (long i= 0; i<n; i++){
...
//until the distance becomes higher that CuttingDistance
{
	...
	//----------------------------------------------
	NumGroups= (long)CandidateVP.size();
...
//		plus the number of edges
{
	...
	while(SizeNewCluster < NumGroups){
		...
		long MaxSize= (long)Cluster[0].size() + (long)ParallelEdges[0].size();
		long Position= 0;
		for(long i= 1; i<(long)Cluster.size(); i++){
			long CurrentSize= (long)Cluster[i].size() + (long)ParallelEdges[i].size();
...
//The second metric is only used to disambiguate between lines linked to the minimum number of VPs
{
	...
	for (long i= 0; i<n; i++){
		...
		NumVPs.push_back((long)VPsInEdge[i].size());
		Distance[i]/= NumVPs[i];
	}
...
//which may appear as intersecting in strict geometrical calculations
{
	...
	for (long i= 0; i<NumGroups; i++){
		bool ToDelete= false;
		long n= (long)ParallelEdges[i].size();
...
{
	long size= (long)ParallelEdges[NumGroup].size();
