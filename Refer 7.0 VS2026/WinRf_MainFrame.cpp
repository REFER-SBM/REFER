...
void CWinRf_MainFrame::getButtonAndIconSize(CSize& buttonSize, CSize& iconSize)
...
// Creates the toolbars linked to the mainframe
{
	...
	m_wndToolBar_Main.SetSizes(buttonSize, iconSize);
	...
	m_wndToolBar_Reconstruction.SetSizes(buttonSize, iconSize);
	...
	m_wndToolBar_Vectorize2D.SetSizes(buttonSize, iconSize);
	...
	m_wndToolBar_Refine2D.SetSizes(buttonSize, iconSize);
	...
	m_wndToolBar_Detection.SetSizes(buttonSize, iconSize);
	...
	m_wndToolBar_Inflation.SetSizes(buttonSize, iconSize);
	...
	m_wndToolBar_Refine3D.SetSizes(buttonSize, iconSize);
	...
	m_wndToolBar_Tessellate3D.SetSizes(buttonSize, iconSize);
	...
	m_wndToolBar_Config_Cues.SetSizes(buttonSize, iconSize);
	...
	m_wndToolBar_Config_Features.SetSizes(buttonSize, iconSize);
	...
	m_wndToolBar_Windows.SetSizes(buttonSize, iconSize);
...
// Opens the dialog to let the user modify the basic Refer settings
{
	...
	Dlg.SetParams();
	if (Dlg.DoModal() == IDOK){
		CString PathINIOld = ReferApp.GeneralParams.PathINI;
		CString PathExamplesOld = ReferApp.GeneralParams.PathExamples;

		Dlg.GetParams();

		if (ReferApp.GeneralParams.PathINI != PathINIOld) {
			CString message;
			message.Format(_T("Are you sure you want to move REFER folder from %s to %s?"), (LPCTSTR)PathINIOld, (LPCTSTR)ReferApp.GeneralParams.PathINI);
			if (AfxMessageBox(message, MB_YESNO) == IDYES) {
				bool moved = ReferApp.MoveFolder(PathINIOld, ReferApp.GeneralParams.PathINI);
				if(!moved)
					ReferApp.GeneralParams.PathINI = PathINIOld;
			}
			else{
				ReferApp.GeneralParams.PathINI = PathINIOld;
			}
		...
		if (ReferApp.GeneralParams.PathExamples != PathExamplesOld) {
			CString message;
			message.Format(_T("Are you sure you want to move REFER examples folder from %s to %s?"), (LPCTSTR)PathExamplesOld, (LPCTSTR)ReferApp.GeneralParams.PathExamples);
			if (AfxMessageBox(message, MB_YESNO) == IDYES) {
				bool moved = ReferApp.MoveFolder(PathExamplesOld, ReferApp.GeneralParams.PathExamples);
				if (!moved)
					ReferApp.GeneralParams.PathExamples = PathExamplesOld;
			}
			else{
				ReferApp.GeneralParams.PathExamples = PathExamplesOld;
			}
