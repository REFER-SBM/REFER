...
//  Returns TRUE if ends normally (even if nothing known is found)
{
	...
	while (getline(in, linea)){
		if(linea.substr(0, 6)== "Vertex"){
			CVertex Vertice;
			long BeginX= (long)linea.find(':')+1;
			long EndX= (long)linea.find(',', BeginX);
			...
			while(linea.substr(BeginY, 1) == " ")
				BeginY++;
			long EndY= (long)linea.find(' ', BeginY);
			if(EndY<0)
				EndY= (long)linea.size()-1;
			...
			long BeginV1 = (long)linea.find('V')+1;
			long EndV1 = (long)linea.find(' ', BeginV1);
			sscanf_s(linea.substr(BeginV1, EndV1-BeginV1).c_str(), "%d", &E.Tail);

			long BeginV2 = (long)linea.rfind('V')+1;
			long EndV2 = (long)linea.size();
