...
|	  e-mail: mjf@digitais.ist.utl.pt
|
    ...
    if (_enclosingRect == NULL) {
        ...
        if (num < 2) {  // is just a point
		    ...
		    CIPoint *paux = new CIPoint((*pts)[i].x+100,(*pts)[i].y);
	            ...
	            CIPoint paux1(int(p1.x+100*cos(ang+M_PI_2)),int(p1.y+100*sin(ang+M_PI_2)));
	            CIPoint paux2(int(p2.x+100*cos(ang+M_PI_2)),int(p2.y+100*sin(ang+M_PI_2)));
...
+----------------------------------------------------------------------------*/
void CIScribble::writeTo(ofstream& of) 
{
    int ns, np;
    CIStroke *strk;
    CIList<CIPoint *> *pts;
...
double CIScribble::hMovement()
{
    ...
    for (int s=0; s<ns; s++) {
        ...
        for (int p=0; p<np-1; p++) {
            mov += fabs((double)((*pts)[p+1]->x - (*pts)[p]->x));  //pcompany2009
        }
    }
    return mov;
}
...
double CIScribble::vMovement()
{
    ...
    for (int s=0; s<ns; s++) {
        ...
        pts = strk->getPoints();
        for (int p=0; p<np-1; p++) {
            mov += fabs ((double)((*pts)[p+1]->y - (*pts)[p]->y));  //pcompany2009
        }
    }
    return mov;
}
