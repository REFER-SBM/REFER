...
{
	//Check size
	long Mrows= (long)M.size();
	long Mcolumns= (long)M[0].size();
...
{
	//Check sizes
	long Mrows1= (long)M1.size();
	long Mcolumns1= (long)M1[0].size();
	for(long i=1; i<Mrows1; i++){
		assert(Mcolumns1 == M1[i].size());
	}

	long Mrows2= (long)M2.size();
	long Mcolumns2= (long)M2[0].size();
...
{
	long m_rows= (long)M.size();
	long m_cols= (long)M[0].size();
